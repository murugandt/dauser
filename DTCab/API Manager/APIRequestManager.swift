...
class APIRequestManager {
    ...
    var imgProgressDelegate: ImageUploadProgressDelegate?
    ...
    func getProfileDetails(param :Dictionary<String , AnyObject> , completion : @escaping(_ success : Bool , _ jsonObject : ProfileModel?) -> ())
    {
        Globals.shared.dontShowmessage = true
        get(request: clientURLRequestGetMethod(path: RequestMethod.logIn.rawValue)) { (success, object) in
            DispatchQueue.main.async(execute: { () -> Void in
                if success {
                ...
                if let response = response as? HTTPURLResponse, 200...299 ~= response.statusCode {
                    do{
                        if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
                            ...
                            var message = ""
                            let dict: NSDictionary = json["error"] as! NSDictionary
                            if((dict.value(forKey: "code") as! Int) == 1){
